...
async function checkSlide(page) {
  return await page.evaluate(async () => {
    ...
    slide.querySelectorAll('pre, .code-block').forEach((el, i) => {
      ...
      if (/[\u250C\u2510\u2514\u2518\u2502\u2500\u253C\u2554\u2557\u255A\u255D\u2551\u2550]/.test(text)) {
        results.issues.push({ type: 'ascii-box-drawing', severity: (slideType === 'code-example') ? 'info' : 'critical', detail: 'pre/code-block contains box-drawing chars. First 150 chars: ' + text.substring(0, 150).replace(/\n/g, '\\n') });
      }
      if (/\*\*[^*]+\*\*/.test(text) && slideType !== 'code-example') {
        results.issues.push({ type: 'literal-markdown-asterisks', severity: 'important', detail: 'pre/code-block contains literal **bold** markdown' });
      }
    });
    const fontChecks = [
...
(async () => {
  ...
  for (const slideNum of TARGET_SLIDES) {
    await page.evaluate((num) => {
      ...
      const target = document.querySelector('.slide[data-number="' + num + '"]');
      if (target) {
        target.classList.add('active');
        const counter = document.querySelector('#slide-counter');
        if (counter) counter.textContent = num + ' / ' + slides.length;
      }
    }, slideNum);
    ...
    console.log('Fonts: title=' + (result.fonts?.title ?? 'N/A').substring(0,40) + ' | body=' + (result.fonts?.body ?? 'N/A').substring(0,40) + ' | code=' + (result.fonts?.code ?? 'N/A').substring(0,40));
    console.log('');
  }
