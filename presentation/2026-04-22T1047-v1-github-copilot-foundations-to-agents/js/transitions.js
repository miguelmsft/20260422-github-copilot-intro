...
function getTransitionType(slide) {
  ...
  switch (type) {
    ...
    case 'transition': case 'recap': case 'break': return 'dissolve';
...
export function transitionSlide(oldSlide, newSlide, direction, onComplete) {
  ...
  switch (transType) {
    case 'zoom':
      gsap.set(newSlide, { opacity: 0, scale: 0.92, xPercent: 0, x: 0, y: 0 }); break;
    case 'dissolve':
      gsap.set(newSlide, { opacity: 0, xPercent: 0, x: 0, y: 0, scale: 1 }); break;
    case 'fade-scale':
      gsap.set(newSlide, { opacity: 0, scale: 0.97, y: 12, xPercent: 0, x: 0 }); break;
    default:
      gsap.set(newSlide, { opacity: 0, xPercent: 100 * xDir, x: 0, y: 0, scale: 1 }); break;
  }
